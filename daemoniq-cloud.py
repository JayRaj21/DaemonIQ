#!/usr/bin/env python3
"""
DaemonIQ — Linux Troubleshooting Assistant (Groq edition)
Single-file edition: daemon + CLI + distro detection all in one.
Uses the Groq cloud API — free tier available, extremely fast inference,
no local GPU or heavy hardware required.
...
Requires: Python 3.8+, GROQ_API_KEY environment variable.
...
# ══════════════════════════════════════════════════════════════════════════════
# DAEMON — background server (Unix socket, session state, Groq API)
# ══════════════════════════════════════════════════════════════════════════════
...
#
# Recommended models (all free on Groq):
#   llama-3.3-70b-versatile   — best quality, strong reasoning      (default)
#   llama-3.1-8b-instant      — fastest responses, lower RAM cost
#   mixtral-8x7b-32768        — strong at long context tasks
#   gemma2-9b-it              — Google Gemma, good for instructions
#
...
def _check_groq(api_key: str) -> tuple[bool, str]:
    """Return (available, message). Validates the Groq key is present."""
    if not api_key:
        return False, (
...
def _call_api(sid: str, message: str, api_key: str = "") -> str:
    ...
    try:
        ...
        if not reply:
            reply = "✗ Empty response from Groq."
        ...
        try:
            ...
            msg = body[:200]
        log.error(f"Groq HTTP {e.code}: {msg}")
        if e.code == 401:
...
def _handle_client(conn):
    try:
        ...
        if cmd == "ping":
            ...
            raw_reply = _call_api(sid, req.get("message", ""), key)
...
def _show_status():
    ...
    print(f"  AI:        {C.CYAN}{r.get('ai_backend', 'Groq')}{C.RESET}")
...
# ── Interactive REPL ──────────────────────────────────────────────────────────
def _repl(session: str, api_key: str = "", auto_exec: bool = False):  # api_key = GROQ_API_KEY
    print(_banner())
    ...
    print(f"{C.MGRAY}Model: {C.CYAN}{GROQ_MODEL}{C.RESET} via Groq  |  Type {C.CYAN}help{C.MGRAY} for commands  |  {C.CYAN}Ctrl+C{C.MGRAY} to exit{C.RESET}")
    _sep()
...
def main():
    ...
    ok_groq, groq_msg = _check_groq(api_key)
    if not ok_groq:
        print(f"{C.RED}✗ Groq not available:{C.RESET}")
        for line in groq_msg.split("\n"):
            print(f"  {line}")
        sys.exit(1)
    ...
    if args.message:
        ...
        r = _request({
            ...
            "session": args.session, "api_key": api_key,
