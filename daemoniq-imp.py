...
# ── Display helpers ───────────────────────────────────────────────────────────
def _show_hardware():
    """Display the hardware snapshot collected at demon startup."""
    if not _daemon_running():
        # Try loading from disk if demon isn't running
        try:
...
def _setup_ollama(model: str):
    ...
    if not _ollama_running():
        print(f"  {C.DIM}Starting Ollama...{C.RESET}")
        _start_ollama_bg()
...
def _arrow_select(prompt: str, options: list, default: int = 0) -> int:
    """
    Interactive arrow-key menu. Returns the index of the selected option.
    Falls back to numbered input if the terminal does not support raw mode.
    """
    import sys, tty, termios

    def _draw(selected: int):
        # Move cursor up to redraw
        sys.stdout.write(f"\033[{len(options)}A")
        for i, opt in enumerate(options):
            if i == selected:
                marker = f"{C.RED}{C.BOLD} ▶  {opt}{C.RESET}"
            else:
                marker = f"    {opt}"
            sys.stdout.write(f"\r{marker}\033[K\n")
        sys.stdout.flush()
    ...
    for i, opt in enumerate(options):
        if i == default:
            sys.stdout.write(f"{C.RED}{C.BOLD} ▶  {opt}{C.RESET}\n")
        else:
    ...
    try:
        ...
        # Non-interactive terminal — fall back to numbered input
        sys.stdout.write(f"\n")
        for i, opt in enumerate(options):
            sys.stdout.write(f"  {C.CYAN}{i+1}){C.RESET} {opt}\n")
        while True:
            try:
                raw = input(f"  {C.GREEN}Choice [1-{len(options)}, default {default+1}]:{C.RESET} ").strip()
                if raw == "":
                    selected = default; break
                if raw.isdigit() and 1 <= int(raw) <= len(options):
                    selected = int(raw) - 1; break
            except (KeyboardInterrupt, EOFError):
                raise KeyboardInterrupt
...
def run_setup():
    ...
    # Default to the detected distro; fall back to Ubuntu (1) if nothing detected
    default_idx = next((i for i, (k, _) in enumerate(DISTROS, 1) if k == detected), 1)
    prompt_hint = f"1-{len(DISTROS)}, default {default_idx}"

    try:
        distro_names = [
            f"{label}{'  ← detected' if key == detected else ''}"
            for key, label in DISTROS
        ]
        distro_idx = _arrow_select("Use arrow keys to select your distro, Enter to confirm", distro_names, default_idx - 1) + 1
    except (KeyboardInterrupt, EOFError):
    ...
    backend = {0: "imp", 1: "sovereign"}[tier_idx]
    cfg["backend"] = backend

    is_qwen = (backend == "sovereign")
    model   = "qwen2.5:14b" if is_qwen else "llama3"
    target  = "daemoniq-sovereign.py" if is_qwen else "daemoniq-imp.py"
    pattern = (r'OLLAMA_MODEL = "qwen2.5:14b"' if is_qwen
               else r'OLLAMA_MODEL = "llama3"')

    if is_qwen:
        print(f"\n{C.CYAN}{C.BOLD}  How much RAM does your machine have?{C.RESET}\n")
        print(f"  {C.CYAN}1){C.RESET} 8 – 12 GB    {C.DIM}(qwen2.5:7b  — ~5GB){C.RESET}")
        print(f"  {C.CYAN}2){C.RESET} 12 – 24 GB   {C.DIM}(qwen2.5:14b — ~9GB){C.RESET}  {C.YELLOW}recommended{C.RESET}")
        print(f"  {C.CYAN}3){C.RESET} 24 GB+       {C.DIM}(qwen2.5:32b — ~20GB){C.RESET}")
        print(f"  {C.CYAN}4){C.RESET} Not sure     {C.DIM}(qwen2.5:7b  — safe default){C.RESET}")
        print()
        ram_options = [
            "8 – 12 GB   (qwen2.5:7b  — ~5GB download)",
            "12 – 24 GB  (qwen2.5:14b — ~9GB download)  recommended",
            "24 GB+      (qwen2.5:32b — ~20GB download)",
            "Not sure    (qwen2.5:7b  — safe default)",
        ]
        try:
            sz_idx = _arrow_select("How much RAM does your machine have?", ram_options, 1)
        except (KeyboardInterrupt, EOFError):
            sz_idx = 1
        model = {0: "qwen2.5:7b", 1: "qwen2.5:14b", 2: "qwen2.5:32b", 3: "qwen2.5:7b"}[sz_idx]
        pattern = r'OLLAMA_MODEL = "qwen2.5:14b"'
    ...
    tier   = {"imp": "Imp", "sovereign": "Sovereign"}[backend]
    mdl    = cfg.get("ollama_model", "")
...
def run_version():
    ...
    if vdata.get("last_updated"):
        print(f"  Last updated: {C.DIM}{vdata['last_updated']}{C.RESET}")
    if os.path.exists(CHANGELOG_FILE):
        print(f"  Changelog:    {C.DIM}{CHANGELOG_FILE}{C.RESET}")
    if os.path.exists(BACKUP_DIR):
        baks = [f for f in os.listdir(BACKUP_DIR) if f.endswith(".bak")]
        if baks:
            print(f"  Backups:      {C.DIM}{len(baks)} in {BACKUP_DIR}{C.RESET}")
    print(f"  Releases:     {C.CYAN}{REPO_URL}/releases{C.RESET}")
    print()


def run_update(patch_path: str = ""):
    """
    Apply a manually downloaded patch file.

    Usage:
      daemoniq update /path/to/daemoniq-imp.py

    To get a patch:
      1. Download the new script from:
             {REPO_URL}/releases
      2. Run: daemoniq update /path/to/downloaded/daemoniq-imp.py
    """
    if not patch_path:
        # No patch provided — show instructions
        this_filename = os.path.basename(__file__)
        print(f"""
  {C.CYAN}{C.BOLD}How to update {PRODUCT_NAME}{C.RESET}

  Automatic updates are not yet enabled.
  To update manually:

  {C.BOLD}1.{C.RESET} Download the latest release from:
     {C.CYAN}{REPO_URL}/releases{C.RESET}

  {C.BOLD}2.{C.RESET} Apply the patch:
     {C.CYAN}{CLI_COMMAND} update /path/to/{this_filename}{C.RESET}

  {C.BOLD}3.{C.RESET} Restart the demon:
     {C.CYAN}{CLI_COMMAND} restart{C.RESET}

  Current version: {C.CYAN}v{PRODUCT_VERSION}{C.RESET}
""")
        return

    # Patch path provided — apply it
    import shutil as _shutil, datetime

    patch_path = os.path.expanduser(patch_path)

    if not os.path.exists(patch_path):
        print(f"  {C.RED}✗{C.RESET} File not found: {patch_path}\n")
        return

    # Read and validate the patch file
    try:
        new_content = open(patch_path).read()
    except Exception as e:
        print(f"  {C.RED}✗{C.RESET} Could not read patch file: {e}\n")
        return

    # Check it's valid Python before doing anything
    try:
        import warnings as _w
        with _w.catch_warnings():
            _w.simplefilter("ignore")
            __import__("ast").parse(new_content)
    except SyntaxError as e:
        print(f"  {C.RED}✗{C.RESET} Patch file has a syntax error: {e}")
        print(f"  {C.DIM}Your current version is unchanged.{C.RESET}\n")
        return

    # Extract version from the patch file
    import re as _re
    m = _re.search(r'PRODUCT_VERSION\s*=\s*"([^"]+)"', new_content)
    patch_version = m.group(1) if m else "unknown"

    if _parse_semver(patch_version) <= _parse_semver(PRODUCT_VERSION):
        print(f"  {C.YELLOW}⚠{C.RESET} Patch version (v{patch_version}) is not newer than "
              f"installed version (v{PRODUCT_VERSION}).")
        try:
            ans = input(f"  Apply anyway? [y/n]: ").strip().lower()
        except (KeyboardInterrupt, EOFError):
            print(); return
        if ans != "y":
    ...
    this_filename = os.path.basename(__file__)
    print(f"\n  Applying patch: v{PRODUCT_VERSION} → v{patch_version}")

    # Back up current version
    backup_path = _backup_current(this_filename)
    if backup_path:
        print(f"  {C.GREEN}✓{C.RESET} Backed up current version")
        print(f"  {C.DIM}  → {backup_path}{C.RESET}")
    else:
    ...
    try:
        ...
        print(f"  {C.RED}✗{C.RESET} Could not write patch: {e}")
    ...
    vdata["current_version"]  = patch_version
...
def run_uninstall():
    ...
    Stops the demon, removes all installed files, the launcher,
...
def run_update(force: bool = False):
    ...
    The remote manifest is a JSON file at UPDATE_BASE_URL/manifest.json:
    {
        ...
        "variants": {
            ...
            "daemoniq-imp.py":  "daemoniq-imp.py",
            "daemoniq-sovereign.py":  "daemoniq-sovereign.py"
        },
...
def main():
    # Internal: demon bootstrap modes (called by subprocess)
    ...
    if args.message in SUBCMDS:
        ...
        if cmd == "start":
            ...
            # daemoniq update [/path/to/patch.py]
            # If a second argument was passed after "update", use it as the patch path
            import sys as _sys
            patch = _sys.argv[2] if len(_sys.argv) > 2 else ""
            run_update(patch)
        elif cmd == "rollback":
            run_rollback()
        elif cmd == "uninstall":
